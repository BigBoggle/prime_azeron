// Thumbstick code taken from replicazeron repo
...
thumbstick_polar_position_t get_thumbstick_polar_position(int16_t x,
                                                          int16_t y) {
  ...
  position.distance =
      (double)sqrt((double)x * (double)x + (double)y * (double)y);
...
#ifdef THUMBSTICK_DEBUG
  dprintf("distance: %5d angle: %5d\n", thumbstick_polar_position.distance,
          thumbstick_polar_position.angle);
#endif
...
#ifdef THUMBSTICK_DEBUG
  dprintf("w: %2d a: %2d s: %2d d: %2d\n", wasd_state.w, wasd_state.a,
          wasd_state.s, wasd_state.d);
#endif
